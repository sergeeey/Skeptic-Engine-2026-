...
### S1. Adversarial Robustness

We tested whether a fabricator aware of Benford's Law could evade detection by forcing first-digit compliance in their synthetic data. Two adversarial strategies were implemented:

**Benford-aware (first-digit only):** For each cell, nonzero values were redistributed to match the theoretical Benford first-digit distribution. Result: first-digit-only detector AUC dropped (partial evasion), but full fusion AUC remained high — cell-level structural features and second-digit statistics caught the adversarial fabrication.

**Benford-aware (first + second digit):** Additionally forced second-digit uniformity. Result: still detected by fusion, because cell-level structural coherence was destroyed during digit redistribution.

**Conclusion:** The fusion approach is robust to adversarial digit manipulation because it combines digit-level features with structural features that cannot be preserved during digit redistribution.

### S2. Cross-Tissue Results (Human PBMC vs Mouse Brain)

| Direction | Resample AUC | Noise AUC | Random NB AUC |
|-----------|-------------|-----------|---------------|
| PBMC to Brain | 0.38 | 1.00 | 0.39 |
| Brain to PBMC | 0.44 | 1.00 | 0.69 |

Cross-tissue transfer failed for all non-trivial artifact types. This is expected: human PBMC and mouse brain have fundamentally different count distributions, gene expression profiles, and sparsity patterns.

### S3. Bootstrap Confidence Intervals

Primary claim (PBMC3k resample, RF fusion): AUC 0.978, 95% CI [0.988, 0.997] (1,000 bootstrap iterations, stratified resampling).

| Experiment | AUC | 95% CI Lower | 95% CI Upper |
|-----------|-----|-------------|-------------|
| H24 Resample RF | 0.978 | 0.988 | 0.997 |
| H24 Random NB RF | 0.999 | 0.994 | 1.000 |
| H23 RPP LR | 0.729 | 0.417 | 0.923 |
| H23 Statcheck LR | 0.765 | 0.452 | 1.000 |

Note: H23 CIs are wide due to small sample sizes (n=59-99).

### S4. Normalized Cross-Validation

Four normalization strategies were tested to rescue cross-dataset generalization:

| Normalization | Resample (mean cross-AUC) | Random NB (mean cross-AUC) |
|--------------|--------------------------|---------------------------|
| Raw | 0.456 | 0.475 |
| Z-score | 0.472 | 0.510 |
| Rank (quantile) | 0.483 | 0.501 |
| Delta-Benford | 0.461 | 0.489 |

None rescued cross-dataset generalization. The failure is fundamental: dataset-specific fingerprints dominate artifact signals in the feature space.

---

## Author Contributions

**S.V.B.:** Conceptualization, methodology, software, formal analysis, writing - original draft, data curation, visualization.
**[Co-author TBD]:** Domain validation, biological interpretation, writing - review & editing.
...
This work was conducted independently without institutional funding. The author thanks the 10x Genomics, Kang et al. (2018), and Bradshaw et al. (2021) groups for making their data publicly available.
